...
class Logger {
  ...
  static info(logTitle, argHash = {}) {
    Logger.#log('info', logTitle, argHash);
  }
  ...
  static debug(logTitle, argHash = {}) {
    Logger.#log('debug', logTitle, argHash);
  }
  ...
  static error(logTitle, argHash = {}) {
    Logger.#log('error', logTitle, argHash);
  }
  ...
  static #log(logType, logTitle, argHash = {}) {
    ...
    const logMessage = Logger.#buildMessage(allArgs);
    Logger.#writeToLog(logType, logTitle, logMessage, argHash);
  }
  ...
  static #writeToLog(logType, logTitle, logMessage, argHash = {}) {
    if (argHash && argHash.wrap === 'start') {
      winstonLogger[logType](Logger.#generateWrapStr(logTitle, 'START'));
    } else if (argHash && argHash.wrap === 'end') {
      winstonLogger[logType](Logger.#generateWrapStr(logTitle, 'END'));
    } else {
  ...
  static #generateWrapStr(logTitle, wrapWord) {
    return `${'='.repeat(40)}[${logTitle}]${'='.repeat(40)}`;
  }
